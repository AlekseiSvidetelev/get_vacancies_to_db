from src.database_updater import (create_database, create_table_database, entry_employer_to_database,
                                  record_vacancies_in_database)
from src.interaction_database import DBManager
from src.utils import (clean_split_str, format_count_vacancies_user_output, format_vacancies_info_user_output,
                       get_list_employers, parser_and_sorted_employers)
...
def user_interaction() -> None:
    ...
    try:
        ...
        top_employers = input("Отсортировать вакансии по максимальному количеству открытых вакансий(да/нет): ")
        ...
        if input("Вывести найденные вакансии? (да/нет): ").lower() == "да":
        ...
        if input("Вывести вакансии с заработной платой выше среднего? (да/нет): ").lower() == "да":
            print(f"Средняя заработная плата по вакансиям: {result.get_avg_salary()}")
            format_vacancies_info_user_output(result.get_vacancies_with_higher_salary())

        # Фильтрация вакансий по ключевым словам
        if input("Отфильтровать вакансии по ключевым словам? (да/нет)").lower() == "да":
